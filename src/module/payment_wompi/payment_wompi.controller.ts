...
export class PaymentWompiController {
  ...
  async makePayment(
    ...
    return await this.paymentWompiService.processTransaction(
      paymentDto.amount,
      paymentDto.userPaymentSourceId,
      paymentDto.destinationAccount,
      paymentDto.appAccount,
    );
  }

  @Post('unlock-product')
  async unlockProduct(
    @Body() unlockDto: { productId: string; userId: string },
  ) {
    return await this.paymentWompiService.unlockProduct(
      unlockDto.productId,
      unlockDto.userId,
    );
  }
  ...
  async getTokenCard(@Body() tokenCardDto: TokenCardDto) {
    const token = await this.paymentWompiService.getTokenCard(
      tokenCardDto.number,
      tokenCardDto.cvc,
      tokenCardDto.expMonth,
      tokenCardDto.expYear,
      tokenCardDto.cardHolder,
    );
    const { id } = token.data;
  ...
  async getMerchats() {
    ...
    const presigned_personal_data_auth =
      token.data.presigned_personal_data_auth;
    return {
